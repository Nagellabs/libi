...
import { renderFrame, getSceneAtFrame } from "@/lib/engine/renderer";
...
describe("renderFrame with empty scenes", () => {
  it("getSceneAtFrame returns a null sentinel and does not throw when scenes is empty", () => {
    ...
    let info!: ReturnType<typeof getSceneAtFrame>;
    expect(() => {
      info = getSceneAtFrame(comp, 0);
    }).not.toThrow();
    expect(info.scene).toBeNull();
    expect(info.sceneIndex).toBe(-1);
  });

  it("fills the background and runs the overlay path with scenes:[] and one overlay", () => {
    const comp: Composition = {
      id: "c",
      name: "c",
      width: 100,
  ...
  it("fills the default-black background with scenes:[] and no overlays and does not throw", () => {
    const comp: Composition = {
      id: "c",
      name: "c",
      width: 100,
