...
describe("renderIndexTable", () => {
  it("renders a markdown table sorted by id with covers + path", () => {
    ...
    expect(md).toMatch(/\| id \| title \| skills \| mcps \| invariants \| covers \| path \|/);
  ...
  /**
   * Half the suite (35 of 69 at the 2026-09-10 sweep) declares no hard invariants, and
   * nothing anywhere said so — an assertion-free scenario looked exactly like one with
   * teeth. The index is where you pick scenarios to run, so the count belongs here.
   */
  it("counts each scenario's invariants and marks the empty ones NONE", () => {
    const md = renderIndexTable([
      scn({ id: "with-teeth", assertions: [{ tool: "run_model", expect: "present" }] }),
      scn({ id: "vacuous", assertions: [] }),
    ]);
    expect(md).toMatch(/\| vacuous \|[^\n]*\| \*\*NONE\*\* \|/);
    expect(md).toMatch(/\| with-teeth \|[^\n]*\| 1 \|/);
    expect(md).toContain("**1 of 2 scenarios declare no hard invariants.**");
  });
});
