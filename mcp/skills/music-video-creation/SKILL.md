...
- If the user has tried 4+ generations in one session and still isn't
  satisfied, suggest a paid `music` provider (see
  `references/providers/<id>.md` under this skill) rather than burning
  another ACE-Step run — with the cost disclosed and their approval,
